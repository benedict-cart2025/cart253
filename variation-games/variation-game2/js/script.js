...
function updateQuiz() {
  ...
  startTimer(timeValue);
  startTimerLine(widthValue);
  timeText.textContent = "tfeL emiT";
...
function showResult() {
  ...
  if (userScore > 3) {
    scoreTag = `<span><p>${questions.length}</p> fo tuo <p>${userScore}</p> tog uoY 🎉 !stargnoc dna </span>`;
  } else if (userScore > 1) {
    scoreTag = `<span><p>${questions.length}</p> fo tuo <p>${userScore}</p> tog uoY ,😎 ecin dna </span>`;
  } else {
    scoreTag = `<span><p>${questions.length}</p> fo tuo <p>${userScore}</p> ylno tog uoY ,😐 yrros dna </span>`;
  }
...
function queCounter(index) {
  let totalQueCounTag = `<span><p>${questions.length}</p> fo <p>${index}</p> noitseuQ</span>`;
